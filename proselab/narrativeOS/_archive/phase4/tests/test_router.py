...
@pytest.fixture
def mock_anthropic() -> MockProvider:
    clear_providers()
    mock = MockProvider(name="anthropic")
    register_provider("anthropic", mock)
    yield mock
    clear_providers()
...
class TestRouterBasics:
    def test_calls_provider_on_l1_miss(self, mock_anthropic: MockProvider, cache):
        llm_call(
    ...
    def test_l1_hit_skips_provider(self, mock_anthropic: MockProvider, cache):
        llm_call(
            role="continuity_extractor",
            system="sys",
            user_message="user",
    ...
    def test_different_inputs_different_keys(
        self, mock_anthropic: MockProvider, cache,
    ):
        llm_call(
            role="continuity_extractor",
            system="sys",
    ...
    def test_passes_cached_blocks_to_provider(
        ...
        assert mock_anthropic.last_request.cached_blocks == ["manuscript text here"]

    def test_tier_override_changes_model(self, mock_anthropic: MockProvider, cache):
        # Override to T3_fast (google:gemini-3-flash)
        # Will fail to find provider since we only registered anthropic mock
...
class TestProviderRegistration:
    def test_get_provider_returns_singleton(self):
        clear_providers()
        # Without registering, get_provider("anthropic") returns real provider
