...
async function generate(scene, voiceProfile, selectedTechs, fb) {
  const sys = buildGeneratorSys(voiceProfile, selectedTechs);
  let msg = "Write one original paragraph for this scene:\n\n" + scene;
  if (fb) msg = "PREVIOUS ATTEMPT REJECTED.\nAnnotation: " + fb.annotation + "\nRewrite instruction: " + fb.rewrite_instruction + "\n\nThe safe version already failed. Apply the techniques more boldly in service of this writer's voice. Do not drift toward any named author.\n\nScene: " + scene;
  return callAPI(sys, msg);
}

async function critique(para) {
  const raw = await callAPI(CRIT_SYS, "Evaluate this paragraph for craft quality. It is an original work -- do not evaluate it against any named author's style. Evaluate it against the failure mode taxonomy and scoring rubric only:\n\n" + para);
